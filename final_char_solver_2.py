# -*- coding: utf-8 -*-
'''
Convolutional char-level deep neural network для решения задачи конкурса
...
merged_left = keras.layers.concatenate([left_seq1, left_seq2, left_seq3, left_seq4, left_seq5, left_seq6])
merged_right = keras.layers.concatenate([right_seq1, right_seq2, right_seq3, right_seq4, right_seq5, right_seq6])

diff = Lambda(lambda x: K.abs(x[0] - x[1]), output_shape=(nb_filters*6,))([merged_left, merged_right])
mul = Lambda(lambda x: x[0] * x[1], output_shape=(nb_filters*6,))([merged_left, merged_right])
...
#model.compile(optimizer=opt, loss=customized_loss1, metrics=['accuracy'])
model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])

#open('chars_siamese2.arch', 'w').write(model.to_json())

print(model.summary())

if DO_TRAIN:
    model_checkpoint = ModelCheckpoint('chars_siamese2.model', monitor='val_loss',
...
if MAKE_SUBMISSION:
    ...
    model.load_weights('chars_siamese2.model')
    y1 = model.predict([X1_submit, X2_submit])[:, 0]

    if DOUBLE_DATASET:
        y2 = model.predict([X2_submit, X1_submit])[:, 0]

    submission_filename = os.path.join(submission_folder, 'submission_chars_2.dat')
